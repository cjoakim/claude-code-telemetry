# claude-code-telemetry

Exploratory repo on how to capture **Claude Code** usage telemetry with 
**OpenTelemetry (OTEL)** receivers, both on localhost and in Azure.

Support for **Cursor** and other AI tools may be added to this repo at a later date.

This repo has been developed on macOS.  Both bash and WindowsPowershell scripts
exist in the repo, but the solutions have not yet been tested on Windows.

**THIS REPO IS CURRENTLY A WORK-IN-PROGRESS.  EXPECTED COMPLETION DATE IS JUNE 30, 2026.**

## The Problem to Solve

- Claude Code is an excellent AI tool/assistant
- However, it provides little insight into when it calls LLM(s)
  - This includes the prompts, the invoked models, and the token utilization
- It also provides little insight on how/when/if Skills and Sub-agents are called
- Tools such as [ccusage](https://ccusage.com/) are useful but they are not granular enough

### Solution Goals

- Collecting and viewing this telemetry either locally, or to Azure
- Utilize the [OpenTelemetry (OTEL) standard](https://opentelemetry.io/)
- If custom code is required, then use Python

### Solutions in this Repo

This repo explores three ways to solve this problem:
- 1. Per the Claude Code documentation, send OTEL telemetry to a localhost container
- 2. Per the Claude Code documentation, send OTEL telemetry to Azure
- 3. With a custom python-based solution to capture and emit  OTEL telemetry

### System Requirements

- **Claude Code**
- **Docker Desktop** (for localhost telemetry receivers)
- **Azure App Insights** and **Azure Log Analytics** (for telemetry sent to Azure)
- **Python 3.13** (for the custom solution)
- **uv** python package manager, see https://docs.astral.sh/uv/
- **jq** - like sed for json, see https://jqlang.org/

---

## Solution 1 : Implement OTEL to localhost per the Claude Code Documentation

See https://code.claude.com/docs/en/monitoring-usage


### Environment Variables

Per the above Claude documentation, set these environment variables on your system
to emit Claude telemetry to the OpenTelemetry Docker Container described below.

```
CLAUDE_CODE_ENABLE_TELEMETRY=1
CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1
OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318
OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf
OTEL_LOG_USER_PROMPTS=1
OTEL_LOGS_EXPORT_INTERVAL=5000
OTEL_LOGS_EXPORTER=otlp
OTEL_METRICS_EXPORT_INTERVAL=10000
OTEL_METRICS_EXPORTER=otlp
OTEL_TRACES_EXPORT_INTERVAL=10000
OTEL_TRACES_EXPORTER=otlp
```

After these environment variables have been set **restart Claude Code**.

### Localhost OpenTelemetry Docker Container

See https://opentelemetry.io/docs/collector/quick-start/

In your shell program, start the collector with Docker Compose:

```
docker compose -f otel-compose.yml up
```

See the **otel-compose.yml** and **otel-collector-config.yml** regarding
the Docker desktop configuration.  Notice how this configuration logs
the collected telemetry to the local **otel-logs/** directory.

Open Claude Code and enter any prompt, such as "What is the capitol of France?".

You will then see that telemetry has been emitted to file **otel-logs/telemetry.jsonl**.
The format of this file is JSON-document-per-line, or JSONL.  The **jq** program
can be used to pretty-print this telemetry.

```
cat telemetry.jsonl | tail -1 | jq
```

While this Docker container provides a UI at port 55679, it appears
to be primitive and difficult to use.

Stop the container with is command:
```
docker compose -f otel-compose.yml down
```

Also exit then restart your Claude session.

### Alternative Localhost Implementation with the Jaeger Container

**Jaeger** is another implementation of an OTEL collector.  See the docs here:
- https://www.jaegertracing.io/
- https://www.jaegertracing.io/docs/2.19/

Jaeger has a nicer and easier to use UI, IMO.

Start the Docker container with this command:
```
docker compose -f jaeger-compose.yml up
```

Reopen Claude Code and enter a prompt like the following:
<p align="center">
   <img src="docs/img/capital-of-belgium.png" width="30%">
</p>

Then visit **http://localhost:16686/** with your browser and enter
search criteria like the following, then click "Find Traces".

<p align="center">
   <img src="docs/img/jaeger-ui-1.png" width="80%">
</p>

You should then be able to click-into a search result and see detail like the following.
You can click into these traces to see the token utilization, models used, and elapsed times.

<p align="center">
   <img src="docs/img/jaeger-ui-2.png" width="80%">
</p>

Be sure to stop the Docker container when appropriate:
```
docker compose -f jaeger-compose.yml down
```

Be sure to check that no container are still running with the **docker ps** command,
then stop them with the **docker stop** command as shown below.
```
docker ps
f0480e735356   jaegertracing/all-in-one:latest   "/go/bin/all-in-one-…"   22 minutes ago   Up 22 minutes   0.0.0.

docker stop f0480e735356
f0480e735356
```

---

## Solution 2 : Implement OTEL to Azure per the Claude Code Documentation

This solution is similar to #1, but it sends the telemetry to Azure PaaS
services rather than a localhost process.

Provision instances of **Azure Application Insights** and **Azure Log Analytics**.

Documentation Links:
- https://learn.microsoft.com/en-us/azure/azure-monitor/app/app-insights-overview
- https://learn.microsoft.com/en-us/azure/azure-monitor/containers/opentelemetry-protocol-ingestion

When you provision Azure Application Insights, be sure to enable OTEL as shown below:

<p align="center">
   <img src="docs/img/provision-app-insights-with-otel.png" width="80%">
</p>

Configure the Application Instights **Diagnostic Settings** to send all data
to your Azure Log Analytics workspace:

<p align="center">
   <img src="docs/img/app-insights-diagnostic-settings.png" width="80%">
</p>

As noted in the [above documentation link](https://learn.microsoft.com/en-us/azure/azure-monitor/containers/opentelemetry-protocol-ingestion) **Microsoft Entra authentication** is required.
Therefore, Solution 2 will be explored at a later date.

---

## Solution 3: Custom Python-based solution to emit OTEL telemetry

This solution is implemented in the **python/** directory of this repo.

### Getting Started

- Clone this repo and cd into its' root directory, then cd into the **python/ directory**
- Execute either venv.ps1 (on Windows 11) or venv.sh (bash, on macOS or Linux)
  - This creates the python virtual environment and installs the necessary libraries
- Be sure to have the environment variables shown above, for a localhost OTEL receiver
- Also set environment variable **APPLICATIONINSIGHTS_CONNECTION_STRING**
  - You will find this value in Azure Portal, on the overview page of your Azure Application Insights instance
  - You can optionally set the environment variables for Solution 3 in a **python/.env** file
- Activate the virtual environment
```
.\.venv\Scripts\activate    # Windows
- or - 
source .venv/bin/activate   # bash
```

### Directory structure of the ~/.claude/ directory

Only the interesting parts, for data extraction, are shown here.

```
...
├── history.jsonl    <-- everthing is logged here
...
├── projects
│   ├── -Users-cjoakim-github-claude-code-telemetry-python   <--- this repo
│   ├── -Users-cjoakim-github-zero-to-AI
...
```

#### The history.jsonl file

Claude appears to log all activity to this file.
The following are two pretty-printed example rows.

This data is shallow in nature, but it points to the directory where Claude
was executed from (i.e. = "project") and the Claude "sessionId".
One must use these coordinates to obtain additional information.

```
...
{
  "display": "/exit",
  "pastedContents": {},
  "timestamp": 1781811300132,
  "project": "/Users/cjoakim/github/claude-code-telemetry",
  "sessionId": "f05b3010-d27e-444c-9d20-5a008453d738"
}
{
  "display": "i want claude to emit OTEL telemetry to Azure Application Insights.  Are the environment variables and other configuration correct to do this?",
  "pastedContents": {},
  "timestamp": 1781811631225,
  "project": "/Users/cjoakim/github/claude-code-telemetry",
  "sessionId": "79281e40-81e9-47c0-9854-1c5da0a29c16"
}
...
```

#### The project directory

Display of the fully-qualified directory of this repo on my computer.
```
pwd
/Users/cjoakim/github/claude-code-telemetry
```

Within the **~/.claude/projects** directory, these directories correspond to
the activities executed from this **claude-code-telemetry** repo.
```
-Users-cjoakim-github-claude-code-telemetry
-Users-cjoakim-github-claude-code-telemetry-python
```

Note how Claude replaces the "/" characters with "-" to derive
the directory names with the projects/ directory.

Note also how Claude doesn't make it easy for you to navigate into these
project directories.
```
[~/.claude/projects]$ cd -Users-cjoakim-github-claude-code-telemetry
-bash: cd: -U: invalid option
cd: usage: cd [-L|-P] [dir]

[~/.claude/projects]$ cd "-Users-cjoakim-github-claude-code-telemetry"
-bash: cd: -U: invalid option
cd: usage: cd [-L|-P] [dir]
```

However, this cd syntax works:
```
cd -- -Users-cjoakim-github-claude-code-telemetry
```

Within the project directory are one-to-many jsonl files.
```
ls -al | grep jsonl | cut -c 51-999

 024ffb6b-8346-413d-9ddc-7906d6c32762.jsonl
 17024e2f-1c67-4e38-a64e-f6254def301c.jsonl
 353815c6-b36b-40b9-acad-11875068957e.jsonl
 3b4ccda0-b14c-4f6c-bf96-5e84e6131e65.jsonl
 59c5260d-d1c6-4a95-b879-727d903ad9af.jsonl
 6cf70278-3033-452d-a086-0d82e283d991.jsonl
 79281e40-81e9-47c0-9854-1c5da0a29c16.jsonl    <-- session ID of interest
 7c91e877-eb16-4c13-a5d3-62074c9a406b.jsonl
 7e5a08dd-608b-4046-bc3c-f09d3442326a.jsonl
 934943a7-ff36-4275-899e-d82d52381216.jsonl
 947fe4fe-09b8-4ce6-bf61-e135ac75124e.jsonl
 9b148c3f-9c56-42da-a022-390fe5058010.jsonl
 b88bf459-ec28-48b8-9fe4-8b71961f8267.jsonl
 c404a2bd-e05c-4971-b572-b7dc4094204b.jsonl
 ca5b7797-a592-4c7a-99ed-943ef755df8e.jsonl
 d72a20dd-0920-45cd-a08e-1e05426f1d6a.jsonl
 f05b3010-d27e-444c-9d20-5a008453d738.jsonl
```

Note that **79281e40-81e9-47c0-9854-1c5da0a29c16** is a session ID in the history.jsonl sample shown above.

The content of file  79281e40-81e9-47c0-9854-1c5da0a29c16.jsonl is intentionally not shown
in this repo.  But it contains many entries generated by Claude as it responds to user
requests.  Some of these entries contain model and token utilization, partial sample below:


```
cat 79281e40-81e9-47c0-9854-1c5da0a29c16.jsonl | grep APPLICATIONINSIGHTS_CONNECTION_STRING | tail -1 | jq

{
  "parentUuid": "ffc95a8f-953a-4d70-a421-ba4045ad5857",
  "isSidechain": false,
  "message": {
    "model": "claude-sonnet-4-6",
    "id": "msg_016K1UeZPia3bYvnRxysy6wF",
    "type": "message",
    "role": "assistant",
    "content": [
      {
        "type": "text",
        "text": "Now I have enough to give you a complete answer. ..."
      }
    ],
    "stop_reason": "end_turn",
    "stop_sequence": null,
    "stop_details": null,
    "usage": {
      "input_tokens": 1,
      "cache_creation_input_tokens": 3567,
      "cache_read_input_tokens": 50340,
      "output_tokens": 2557,
      "server_tool_use": {
        "web_search_requests": 0,
        "web_fetch_requests": 0
      },
      "service_tier": "standard",
      "cache_creation": {
        "ephemeral_1h_input_tokens": 3567,
        "ephemeral_5m_input_tokens": 0
      },
      "inference_geo": "not_available",
      "iterations": [
        {
          "input_tokens": 1,
          "output_tokens": 2557,
          "cache_read_input_tokens": 50340,
          "cache_creation_input_tokens": 3567,
          "cache_creation": {
            "ephemeral_5m_input_tokens": 0,
            "ephemeral_1h_input_tokens": 3567
          },
          "type": "message"
        }
      ],
      "speed": "standard"
    },
    "diagnostics": null
  },
  "requestId": "req_011CcBJ5QwW9X9iAioPcQ3kG",
  "attributionMcpServer": "claude.ai Microsoft Learn",
  "attributionMcpTool": "microsoft_docs_fetch",
  "type": "assistant",
  "uuid": "23c3d913-d9e8-4c50-b19a-b55733f2ca87",
  "timestamp": "2026-06-18T19:42:38.160Z",
  "userType": "external",
  "entrypoint": "cli",
  "cwd": "/Users/cjoakim/github/claude-code-telemetry",
  "sessionId": "79281e40-81e9-47c0-9854-1c5da0a29c16",
  "version": "2.1.181",
  "gitBranch": "main"
}
```

#### Conclusion

Precise model and token utilization **can** be extracted from the ~./claude/ directories.

The navigation logic is thus:
- read the events from the ~/.claude/history.jsonl file
  - obtain the project and session id for each event
  - translate the filename "project" value in history.jsonl to a dashed directory name in the projects directory
  - read the appropriate session jsonl file from the project directory
    - iterate the rows of that session.json file
    - parse each for model and utilization data
    - collect this data into a list

**Timestamp** values within these jsonl files can be used by an extraction program 
to avoid duplicate data.  One challenge is that the history.jsonl timestamps
are in epoch (integer) format while other files use a string-based timestamp like "2026-06-18T19:42:38.160Z".

The collected events can then be emitted to your OTEL destination-of-choice python program.
This is described later on this page.

---

### Custom Output Directory for Captured Claude data

The **.claudex/** directory in your HOME directory.
Create it as follows in bash.  Use the equivalent commands in Windows PowerShell.

```
mkdir -p ~/.claudex/data
```

```
~/.claudex/data/
~/.claudex/hooks.jsonl
~/.claudex/hooks-tokens.jsonl
```

---

## Addendum

### ccusage sample output

See https://ccusage.com/

Only Session-level granularity is available.  For example, from **ccusage blocks**.

<p align="center">
   <img src="docs/img/ccusage-blocks.png" width="80%">
</p>
